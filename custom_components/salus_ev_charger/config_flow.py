...
class SalusEvChargerConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_reauth_confirm(
        ...
        if user_input is not None:
            client = SalusApiClient(username=email)
            try:
                await self.hass.async_add_executor_job(client.login, user_input[CONF_PASSWORD])
            except SalusApiError as exc:
                _LOGGER.warning("Salus reauth failed: %s", exc)
                errors["base"] = "invalid_auth"
            except Exception:  # noqa: BLE001
                _LOGGER.exception("Unexpected error during Salus reauth")
                errors["base"] = "unknown"
            else:
